...
function App() {
  ...
  const updateProductData = async (id) => {
    let product;
    // 從 modalType 判斷點擊的是新增/編輯 modal，再執行後續操作，將 modal 整合的做法
    if (modalType === "edit") {
      product = `product/${id}`;
    } else {
      product = `product`;
  ...
  const delProductData = async (id) => {
    try {
      const url = `${API_BASE}/api/${API_PATH}/admin/product/${id}`;
                  ...
                  <tr key={product.id}>
                    ...
                    <td>
                      <div className="btn-group">
                        ...
                        <button type="button" className="btn btn-outline-danger btn-sm" onClick={() => openModal(product, "delete")}>
                          刪除
                        </button>
                      </div>
                    </td>
